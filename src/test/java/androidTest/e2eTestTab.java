...
//			Fill med form
		...
		((JavascriptExecutor) driver).executeScript("mobile: doubleClickGesture",
				ImmutableMap.of("elementId", ((RemoteWebElement) addMedName).getId()));
		addMedName.sendKeys("Lesinopril");

		WebElement Dosage = driver.findElements(By.className("android.widget.EditText")).get(1);
		((JavascriptExecutor) driver).executeScript("mobile: doubleClickGesture",
				ImmutableMap.of("elementId", ((RemoteWebElement) Dosage).getId()));
		Dosage.sendKeys("2");
		...
		((JavascriptExecutor) driver).executeScript("mobile: dragGesture",
				ImmutableMap.of("elementId", ((RemoteWebElement) Frequency).getId(), "endX", 501, "endY", 532));
//			Select Category
...
//     Verify that the average has been updated
		/*
		 * if (Integer.parseInt(SleepTrackerPageObjectFactory.initialAverageSleep) >
		 * Integer.parseInt(SleepTrackerPageObjectFactory.finalAverageSleep)) {
...
//    	    "elementId", ((RemoteWebElement) bpm).getId(),
				"startX", 1073, "startY", 581, "endX", 805, "endY", 589));
...
//	log systolic BP
		Thread.sleep(2000);
		((JavascriptExecutor) driver).executeScript("mobile: dragGesture",
				ImmutableMap.of("startX", 1113, "startY", 550, "endX", 814, "endY", 537));

//	log diastolic BP
		Thread.sleep(2000);
		((JavascriptExecutor) driver).executeScript("mobile: dragGesture",
				ImmutableMap.of("startX", 941, "startY", 1122, "endX", 809, "endY", 1117));
//	select date
		driver.findElements(By.className("android.widget.ImageView")).get(6).click();
		driver.findElement(AppiumBy.accessibilityId("5, Wednesday, June 5, 2024")).click();
		driver.findElement(AppiumBy.accessibilityId("OK")).click();
